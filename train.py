...
# from gaussian_renderer import render, network_gui # 本地验证，注释掉需要cuda的部分
import sys
# from scene import Scene, GaussianModel # 本地验证，注释掉需要cuda的部分
...
def upscale_data(args, device='cpu'):
    ...
    # 上采样操作是在 process_S 和 process_ALS 中完成的
...
def prepare_output_and_logger(args):    
    ...
    return tb_writer
# training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
...
if __name__ == "__main__":
    ...
    args = upscale_data(args) # 进入上采样阶段
    # 利用超分辨率图像训练3DGS模型
    '''解析模型参数， 优化参数， 管道参数， '''
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, \
             args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.lambda_tex, args.subpixel)
